...
//左右圆环判断阈值
int  HUANDAO_Ryuzhi=240;
int  HUANDAO_Lyuzhi=240;
...
void judge(void)
{
	...
	if(flag_circle==1 && adc[3]>0.65 &&circle_R==0&&circle_L==0&&flag_shizi==0&&flag_shizi_R==0
		&&flag_barrier==0 && fabs(error) < 1 && (((cricle_order >> cricle_count_order) & 1)|| cricle_order == 20))		//判断左入环
	{
		circle_L=1;straight_inc=motor_inc;
...
//			circle_inc=(int)fabs(x_inc);
			...
			else if(circle_inc>25&&circle_inc<=360)
			{
				buzzer=1;
...
//			return;
		...
		else if(motor_inc-straight_inc>=700&&motor_inc-straight_inc<2500)
		{
...
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if(flag_circle == 2 && adc[3]>0.65&& circle_R == 0 && circle_L == 0 &&flag_shizi==0&&flag_shizi_R==0
		&& flag_barrier==0 && fabs(error) < 1 && (((cricle_order >> cricle_count_order) & 1)==0|| cricle_order == 20))		//判断右入环
	{
		circle_R=1;
...
//				error=0.65;
	...
	//			circle_inc=(int)fabs(x_inc);
				...
				else if(circle_inc>23&&circle_inc<=360)
				{
	//				error=0.9;
					//error=(1.25*(sqrt(adc[0])-sqrt(adc[6]))+1.25*(sqrt(adc[1])-sqrt(adc[5])))/(adc[1]+adc[5]+adc[6]+adc[0]+1.35*adc[3]);
				}
//				else if(circle_inc>330&&circle_inc<=350)
//				{
//				error=0.52;
//				}
				else
...
//			}
		...
		else if(Lidar1.distance < 80 && Lidar1.distance > 35&& barrier==0 && fabs(error) < 0.7 &&  (((bizhang_order >> barrier_count) & 1)==0))
		{
			barrier=2;
			bizhang_inc=motor_inc;
		}
	...
	if(barrier==2) 
		{
			if(motor_inc-bizhang_inc <= 8000)
			{
					r_target_speed = 24.0;
					l_target_speed = 24.0;
				buzzer = 0;
			}
			else if(motor_inc-bizhang_inc > 8000)
			{
				r_target_speed = 30.0;
				l_target_speed = 30.0;
				buzzer = 1;
...
//		}
		...
		if(bizhang==3)		//走弧线
		{
			if(motor_inc-bizhang_inc <= 600)
			{
				buzzer = 1;
				error = 0.65;
			
			}
			else if (motor_inc-bizhang_inc > 600)
			{
				bizhang_inc = motor_inc;
				bizhang=4;
			}
...
//	 	  0.6,0.082,0.027,0.930,0.15,0.214,0.327};
	...
	//	0.478,0.20,0.035,1.023,0.134,0.178,0.634, 				//第十一版横电感室内横电感180
